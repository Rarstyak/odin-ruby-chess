...
class Pawn < Piece
  ...
  def get_threat(board, coor)
    ...
    if @color == :white
      ...
      list.push(rel_cell_r&.coor) unless rel_cell_r&.piece.nil? || same_color?(rel_cell_r&.piece)
      rel_cell_l = board.get_cell(coor, [-1, 1])
      list.push(rel_cell_l&.coor) unless rel_cell_l&.piece.nil? || same_color?(rel_cell_l&.piece)
      ...
      list.push(rel_cell_ff&.coor) if rel_cell_f&.piece.nil? && rel_cell_ff&.piece.nil? && (coor[1] == 1)
      # ??? kill en passant
    end
    if @color == :black
      # only threaten rel [[1, -1], [-1, -1]] if enemy present
      ...
      rel_cell_ff = board.get_cell(coor, [0, -2])
      list.push(rel_cell_ff&.coor) if rel_cell_f&.piece.nil? && rel_cell_ff&.piece.nil? && (coor[1] == 6)
      # ??? kill en passant
    end
    list.compact
  end
